...
export default function ClientesPage() {
  ...
  return <LayoutShell><div className="space-y-4"><h1 className="text-3xl font-bold">Clientes</h1>
    <form onSubmit={saveCliente} className="card p-4 grid md:grid-cols-3 gap-3">
      ...
      <input className="border rounded p-2" placeholder="Celular" value={form.celular} onChange={(e)=>setForm({...form,celular:e.target.value})} required />
      <input className="border rounded p-2" placeholder="Email" value={form.email} onChange={(e)=>setForm({...form,email:e.target.value})} />
      <input className="border rounded p-2" placeholder="Empresa" value={form.empresa} onChange={(e)=>setForm({...form,empresa:e.target.value})} />
      <select className="border rounded p-2" value={form.etapa_ciclo_vida} onChange={(e)=>setForm({...form,etapa_ciclo_vida:e.target.value})}>{etapas.map(x=><option key={x}>{x}</option>)}</select>
      <input className="border rounded p-2" placeholder="Estado" value={form.estado} onChange={(e)=>setForm({...form,estado:e.target.value})} />
      <input className="border rounded p-2" placeholder="Origen" value={form.origen} onChange={(e)=>setForm({...form,origen:e.target.value})} />
      <input className="border rounded p-2 md:col-span-2" placeholder="Notas" value={form.notas} onChange={(e)=>setForm({...form,notas:e.target.value})} />
      <button className="bg-accent text-white rounded p-2">{editingId ? 'Guardar cambios' : 'Crear cliente'}</button>
    </form>
    <div className="flex gap-3"><input className="border rounded p-2" placeholder="Buscar cliente" value={q} onChange={e=>setQ(e.target.value)} /><select className="border rounded p-2" value={etapa} onChange={e=>setEtapa(e.target.value)}><option value="">Todas las etapas</option>{etapas.map(x=><option key={x}>{x}</option>)}</select></div>
