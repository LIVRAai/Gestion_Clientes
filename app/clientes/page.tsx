...
export default function ClientesPage() {
  ...
  async function load() {
    ...
    const cc = await supabase.from('compras').select('*');
    setCompras((cc.data ?? []) as Compra[]);
  }
  useEffect(() => { load(); }, []);
  ...
  return <LayoutShell><div className="space-y-4"><h1 className="text-2xl md:text-3xl font-bold">Base de clientes</h1>
    ...
    <div className="card overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="bg-blue-50"><tr><th className="p-3 text-left">Nombre</th><th className="text-left">Etapa automática</th><th className="text-left">Estado</th><th className="text-left">Compras</th><th className="text-left">Valor total</th><th className="text-left">Próxima acción</th><th className="text-left">Acciones</th></tr></thead><tbody>{list.map(c=>{const auto=calcularLifecycle(c,compras);return <tr key={c.id} className="border-t"><td className="p-3">{c.nombre}<p className="text-slate-500">{c.empresa}</p></td><td><span className="badge">{auto.etapa_ciclo_vida}</span><p className="text-xs text-slate-400">Manual: {c.etapa_ciclo_vida}</p></td><td><span className="px-2 py-1 rounded bg-blue-100">{auto.estado}</span></td><td>{auto.numero_compras}</td><td>{cop(auto.valor_total_cliente)}</td><td className="text-xs text-slate-600">{auto.proxima_accion_sugerida}</td><td><div className="flex flex-wrap gap-2 py-2"><Link href={`/clientes/${c.id}`} className="btn-secondary !px-3 !py-1.5 text-xs">Ver</Link><button onClick={()=>{setEditingId(c.id);setForm(c);}} className="btn-secondary !px-3 !py-1.5 text-xs">Editar</button><a target="_blank" href={`https://wa.me/57${c.celular}`} className="btn-secondary !px-3 !py-1.5 text-xs !text-green-700">WhatsApp</a><button onClick={()=>removeCliente(c.id)} className="btn-secondary !px-3 !py-1.5 text-xs !text-red-600">Eliminar</button></div></td></tr>})}</tbody></table>{list.length===0 && <div className="p-8 text-center text-slate-500">Aún no tienes clientes. Crea el primero para empezar a gestionar relaciones.</div>}</div></div></LayoutShell>;
}
