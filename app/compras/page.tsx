...
export default function ComprasPage(){
  ...
  return <LayoutShell><div className="space-y-4"><h1 className="text-3xl font-bold">Compras</h1><form onSubmit={save} className="card p-4 grid md:grid-cols-3 gap-3"><select className="border rounded p-2" value={form.cliente_id} onChange={e=>setForm({...form,cliente_id:e.target.value})} required><option value="">Cliente</option>{clientes.map(c=><option key={c.id} value={c.id}>{c.nombre}</option>)}</select><select className="border rounded p-2" value={form.producto_id} onChange={e=>{const prod=productos.find(p=>p.id===e.target.value);setForm({...form,producto_id:e.target.value,valor_unitario:Number(prod?.precio||0)})}} required><option value="">Producto</option>{productos.map(p=><option key={p.id} value={p.id}>{p.nombre}</option>)}</select><input className="border rounded p-2" type="date" value={form.fecha_compra} onChange={e=>setForm({...form,fecha_compra:e.target.value})} required/><input className="border rounded p-2" type="number" min={1} value={form.cantidad} onChange={e=>setForm({...form,cantidad:Number(e.target.value)})} required/><input className="border rounded p-2" type="number" min={0} value={form.valor_unitario} onChange={e=>setForm({...form,valor_unitario:Number(e.target.value)})} required/><input className="border rounded p-2" value={valorTotal} readOnly/><input className="border rounded p-2 md:col-span-2" placeholder="Notas" value={form.notas} onChange={e=>setForm({...form,notas:e.target.value})}/><button className="bg-accent text-white rounded p-2">Registrar compra</button></form>{msg && <p className="text-sm">{msg}</p>}<div className="card overflow-hidden"><table className="w-full text-sm"><thead className="bg-blue-50"><tr><th className="p-3 text-left">Cliente</th><th>Producto</th><th>Fecha</th><th>Cantidad</th><th>Total</th></tr></thead><tbody>{compras.map(c=>{const cli=clientes.find(x=>x.id===c.cliente_id);const pro=productos.find(x=>x.id===c.producto_id);return <tr key={c.id} className="border-t"><td className="p-3">{cli?.nombre}</td><td>{pro?.nombre}</td><td>{c.fecha_compra}</td><td>{c.cantidad}</td><td>${Number(c.valor_total).toLocaleString('es-CO')}</td></tr>})}</tbody></table></div></div></LayoutShell>
}
